...
export async function POST(
	...
	try {
		...
		const body = await request.json() as { creditorId: string };
		if (!body.creditorId || typeof body.creditorId !== "string") {
			return NextResponse.json({ error: "Invalid creditorId" }, { status: 400 });
		}

		await settleAllDebtsToUser(poolId, active.member.id, body.creditorId);
		...
		console.error("Settle debts error:", error);
		return NextResponse.json({ error: "Failed to settle debts" }, { status: 500 });
	}
}
